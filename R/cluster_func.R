...
cluster_func <- function(data1, data2, type1, type2){
  ...
  cluster1 <- igraph::transitivity((as.igraph(qgraph(snet1$pcor_adj, DoNotPlot=T))))

  cluster2 <- igraph::transitivity((as.igraph(qgraph(snet2$pcor_adj, DoNotPlot=T))))
